...
def setup_global_logging():
    ...
    # 日志文件名含当天日期，例如：duty_reminder_2025-01-20.log
    today_str = datetime.now().strftime('%Y-%m-%d')
    log_file = os.path.join(log_dir, f"duty_reminder_{today_str}.log")
...
class DutyReminderApp:
    ...
    def create_main_window(self):
        ...
        buttons = [
            ...
            ("修改条目姓名", lambda w=window: self.rename_duty_person(w)),
    ...
    def rename_duty_person(self, window):
        """修改值日列表中某一条目的姓名"""
        task_key = self.get_selected_task_key()
        task_data = self.task_groups[task_key]
        if not task_data['duty_list']:
            ...
            return
        rename_win = tk.Toplevel(window)
        rename_win.title("修改条目姓名")
        rename_win.geometry("320x200")
        rename_win.transient(window)
        rename_win.grab_set()
        x = window.winfo_rootx() + (window.winfo_width() // 2) - 160
        y = window.winfo_rooty() + (window.winfo_height() // 2) - 100
        rename_win.geometry(f"320x200+{x}+{y}")

        ttk.Label(rename_win, text="选择要修改的人员:", font=("微软雅黑", 10)).pack(pady=(12, 2))
        old_var = tk.StringVar()
        old_combo = ttk.Combobox(rename_win, textvariable=old_var,
                                 values=task_data['duty_list'], state="readonly", width=20)
        old_combo.pack(pady=4)
        # 默认选中当前值日人员
        current = self.get_current_duty_person(task_key)
        if current in task_data['duty_list']:
            old_combo.set(current)

        ttk.Label(rename_win, text="输入新姓名:", font=("微软雅黑", 10)).pack(pady=(8, 2))
        new_entry = ttk.Entry(rename_win, width=22)
        new_entry.pack(pady=4)

        def on_select(event=None):
            """选中旧姓名后自动填入输入框"""
            new_entry.delete(0, tk.END)
            new_entry.insert(0, old_var.get())

        old_combo.bind('<<ComboboxSelected>>', on_select)
        on_select()  # 初始化时也填入
        ...
        def confirm_rename():
            ...
            if not old_name:
                messagebox.showwarning("警告", "请先选择要修改的人员")
                return
            if not new_name:
                messagebox.showwarning("警告", "新姓名不能为空")
                return
            if new_name == old_name:
                messagebox.showinfo("提示", "姓名未发生变化")
                rename_win.destroy()
                return
            if new_name in task_data['duty_list']:
                messagebox.showwarning("警告", f"'{new_name}' 已在值日列表中")
                return
            idx = task_data['duty_list'].index(old_name)
            task_data['duty_list'][idx] = new_name
            self.logger.info(f"【值日管理】任务 {task_data['name']} 条目已修改: {old_name} -> {new_name}")
            self.update_all_floating_displays()
            self.update_main_window_display()
            self.save_data()
            messagebox.showinfo("成功", f"已将 '{old_name}' 修改为 '{new_name}'")
            rename_win.destroy()

        btn_frame = ttk.Frame(rename_win)
        btn_frame.pack(pady=14)
        ttk.Button(btn_frame, text="确认修改", command=confirm_rename).pack(side=tk.LEFT, padx=10)
        ttk.Button(btn_frame, text="取消", command=rename_win.destroy).pack(side=tk.RIGHT, padx=10)
        new_entry.bind('<Return>', lambda e: confirm_rename())
